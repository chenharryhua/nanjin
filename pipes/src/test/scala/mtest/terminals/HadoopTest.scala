...
class HadoopTest extends AnyFunSuite {
  ...
  test("ymdh") {
    ...
    assert(
      hdp.latestYmdh(path).unsafeRunSync().get.pathStr.takeRight(33) === "Year=2023/Month=08/Day=30/Hour=00")
  }
  test("exist") {
    assert(hdp.isExist(p1).unsafeRunSync())
    assert(hdp.locatedFileStatus(path).unsafeRunSync().count(_.isFile) == 3)
  }
}
