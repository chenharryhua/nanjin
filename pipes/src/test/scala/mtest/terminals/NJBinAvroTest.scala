...
class NJBinAvroTest extends CatsEffectSuite {
  import HadoopTestData.*

  def fs2(path: Url, file: BinAvroFile, data: Set[GenericRecord]): IO[Unit] = {
  ...
  test("10.stream concat") {
    ...
    for {
      _ <- hdp.delete(path) >>
        (s ++ s ++ s).through(hdp.sink(path).binAvro).compile.drain
      size <- hdp.source(path).binAvro(100, pandaSchema).compile.fold(0) { case (s, _) =>
        s + 1
      }
