...
class NJProtobufTest extends CatsEffectSuite {
  ...
  def fs2(path: Url, file: ProtobufFile, data: Set[TestAnimal]): IO[Unit] = {
    ...
    for {
      ...
      roundTrip <- hdp.source(tgt).protobuf[TestAnimal](100).compile.toList
  ...
  test("10.stream concat") {
    ...
    for {
      _ <- hdp.delete(path) >>
        (s ++ s ++ s).through(hdp.sink(path).protobuf).compile.drain
      size <- hdp.source(path).protobuf[TestAnimal](100).compile.fold(0) { case (s, _) =>
        s + 1
      }
