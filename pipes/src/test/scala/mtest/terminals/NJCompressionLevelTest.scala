...
/** Tests for compression level combinations that are not covered by other test files.
  ...
  */
class NJCompressionLevelTest extends CatsEffectSuite {
  ...
  test("1.deflate level 9 on text - round trip") {
    ...
    (hdp.delete(tgt) >> action).map { xs =>
      val result = xs.flatMap(io.circe.jawn.decode[Tiger](_).toOption).toSet
      assert(result == data)
    }
  }
  ...
  test("2.deflate level 1 on circe - round trip") {
    ...
    val action = ts.through(sink).compile.drain >> src.compile.toList
    (hdp.delete(tgt) >> action).map { xs =>
      val result = xs.flatMap(_.as[Tiger].toOption).toSet
      assert(result == data)
    }
  }
  ...
  test("3.deflate level 9 on binary avro - round trip") {
    ...
    val action = ts.through(sink).compile.drain >> src.compile.toList
    (hdp.delete(tgt) >> action).map(xs => assert(xs.toSet == data))
  }
