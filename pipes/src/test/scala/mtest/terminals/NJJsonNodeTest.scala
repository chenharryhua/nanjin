...
class NJJsonNodeTest extends CatsEffectSuite {
    ...
    for {
      ...
      roundTrip <-
        hdp
          ...
          .compile
          .toList
    } yield assert(roundTrip.flatMap(jsonNode2GenericRecord(_, pandaSchema).toOption).toSet == data)
  }
  ...
  test("8.jsonNode source skips embedded empty lines") {
    ...
    for {
      ...
      result <- hdp.source(tgt).jsonNode(10, objectMapper.reader()).compile.toList
    } yield {
