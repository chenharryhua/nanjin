...
final class HadoopProtobuf[F[_]] private (
  ...
  def sink[A <: GeneratedMessage](path: NJPath)(implicit F: Sync[F]): Pipe[F, Chunk[A], Nothing] = {
    (ss: Stream[F, Chunk[A]]) =>
      Stream.resource(getWriterR[A](path.hadoopPath)).flatMap(w => ss.foreach(w.write))
  }
