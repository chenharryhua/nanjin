...
sealed trait LikeProducerRecords[F[_, _]] extends Bitraverse[F] {
  ...
  def lens[K1, V1, K2, V2]
    : PLens[F[K1, V1], F[K2, V2], Chunk[ProducerRecord[K1, V1]], Chunk[ProducerRecord[K2, V2]]]

  final def traversal[K1, V1, K2, V2]
    : PTraversal[F[K1, V1], F[K2, V2], ProducerRecord[K1, V1], ProducerRecord[K2, V2]] =
    lens.composeTraversal(
...
object LikeProducerRecords {
  import monocle.function.Each.traverseEach
    ...
    new LikeProducerRecords[Fs2TransactionalProducerRecords[F, *, *, P]] {
      override def lens[K1, V1, K2, V2]: PLens[
        ...
        PLens[
          ...
          Fs2TransactionalProducerRecords[F, K2, V2, P],
          Chunk[ProducerRecord[K1, V1]],
          Chunk[ProducerRecord[K2, V2]]] { tprs =>
          tprs.records.flatMap(_.records.map(isoFs2ProducerRecord.get))
        } { b => s =>
          ???
        }
    }
