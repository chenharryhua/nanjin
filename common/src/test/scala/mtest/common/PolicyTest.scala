...
import com.github.chenharryhua.nanjin.common.chrono.{lazyTickList, policies, TickStatus}
import cron4s.Cron
...
class PolicyTest extends AnyFunSuite {
  ...
  test("policy") {
    ...
    lazyTickList(ts).take(10).foreach { t =>
  ...
  test("policy 2") {
    ...
    lazyTickList(ts).slice(320, 365).foreach(println)
  }
}
