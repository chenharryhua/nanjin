...
class TickStreamTest extends AnyFunSuite {
  test("1.tick") {
    ...
    val ticks = tickStream.tickScheduled[IO](londonTime, (_: Policy.type) => policy)
  ...
  test("2.process longer than 1 second") {
    ...
    val ticks = tickStream.tickScheduled[IO](berlinTime, (_: Policy.type) => policy)
  ...
  test("3.process less than 1 second") {
    val policy = Policy.crontab(crontabs.secondly)
    val ticks = tickStream.tickScheduled[IO](cairoTime, (_: Policy.type) => policy)
