...
class TickStreamTest extends AnyFunSuite {
  test("1.tick") {
    ...
    val ticks = tickStream.past[IO](londonTime, policy)
  ...
  test("2.process longer than 1 second") {
    ...
    val ticks = tickStream.past[IO](berlinTime, policy)
  ...
  test("3.process less than 1 second") {
    val policy = Policy.crontab(crontabs.secondly)
    val ticks = tickStream.past[IO](cairoTime, policy)
  ...
  test("4.constant") {
    ...
    val ticks = tickStream.past[IO](saltaTime, policy)
  ...
  test("5.fixed rate") {
    ...
    val ticks = tickStream.past[IO](darwinTime, policy)
    val sleep: IO[JavaDuration] =
      Random
        .scalaUtilRandom[IO]
  ...
  test("6.giveUp") {
    val ticks = tickStream.past[IO](saltaTime, Policy.giveUp).compile.toList.unsafeRunSync()
    assert(ticks.isEmpty)
  }

  test("7.future tick - giveUp") {
    val ticks = tickStream.future[IO](saltaTime, Policy.giveUp).compile.toList.unsafeRunSync()
    assert(ticks.isEmpty)
  }

  test("8.future tick") {
    val List(a, b, c) =
      tickStream.future[IO](saltaTime, Policy.fixedDelay(1.seconds).limited(3)).compile.toList.unsafeRunSync()
    assert(a.index == 1)
    assert(b.index == 2)
    assert(c.index == 3)
