...
object observers {
  ...
  def emailObserver: EmailObserver[IO] =
    EmailObserver(SimpleEmailService[IO](_.region(Region.AP_SOUTHEAST_2)))
      .withPolicy(Policy.crontab(_.every12Hours).offset(8.hours), sydneyTime)
      .withCapacity(200)

  val cloudwatch: CloudWatchObserver[IO] =
    CloudWatchObserver(CloudWatch[IO](_.region(Region.AP_SOUTHEAST_2))).withP50.withP95.withMax
