...
import com.github.chenharryhua.nanjin.kafka.{AvroTopic, KafkaContext}
...
final class KafkaObserver[F[_]](ctx: KafkaContext[F], translator: Translator[F, Event])(implicit F: Async[F])
  ...
  def observe(topicName: TopicName): Pipe[F, Event, Event] = {
    ...
    val topic = AvroTopic[KJson[EventKey], KJson[Event]](topicName)

    (ss: Stream[F, Event]) =>
      for {
        client <- Stream.resource(ctx.produce(topic.pair).clientR)
