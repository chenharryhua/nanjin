...
class CancellationTest extends AnyFunSuite {
  ...
  test("1.cancellation - canceled actions are failed actions") {
    ...
    assert(c.asInstanceOf[ActionFail].error.throwable.getMessage == "action was canceled")
  ...
  test("8.cancellation - cancel in middle of retrying") {
    ...
    assert(d.asInstanceOf[ActionFail].error.throwable.getMessage == "action was canceled")
    assert(e.isInstanceOf[ServiceStop])
  }
