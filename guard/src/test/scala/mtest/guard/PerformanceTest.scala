...
class PerformanceTest extends AnyFunSuite {
  val service: ServiceGuard[IO] =
    TaskGuard[IO]("performance").service("actions").updateConfig(_.withMetricReport(10.seconds))
...
//  }
  ...
  test("critical - notes") {
    ...
    service.eventStream { ag =>
      val ts =
        ag.action("t", _.critical.withoutTiming.withoutCounting).retry(IO(i += 1)).logOutput(_.asJson).run
      ts.foreverM.timeout(take).attempt
    }.compile.drain.unsafeRunSync()
  ...
  test("notice") {
    var i: Int = 0
    service.eventStream { ag =>
      val ts = ag.action("t", _.notice.withoutTiming.withoutCounting).retry(IO(i += 1)).run
      ts.foreverM.timeout(take).attempt
    }.compile.drain.unsafeRunSync()
