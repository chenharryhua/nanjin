...
/** Tests for `MetricsHubS`, the stream-native adapter over `MetricsHub`. Each method wraps the hub's
  ...
  */
class MetricsHubSTest extends AnyFunSuite {

  private val service: ServiceGuard[IO] =
    TaskGuard[IO]("metrics-hub-s").service("metrics-hub-s")

  private def lastSnapshot(body: com.github.chenharryhua.nanjin.guard.service.Agent[IO] => IO[Unit]) =
    service
      .eventStream(body)
      .map(checkJson)
