...
class CircuitBreakerTest extends CatsEffectSuite {
  ...
  test("3.race") {
    ...
    service
      .eventStream(_.circuitBreaker(maxFailures = 5, _.fixedDelay(1.seconds).repeat).use { cb =>
  ...
  test("4.non-positive maxFailures fails configuration") {
    service
      ...
      .compile
      .lastOrError
      .map(ss => assert(ss.cause.exitCode == 3))
  }

  test("5.reset state") {
    service
      .eventStream(_.circuitBreaker(maxFailures = 1, f = _.fixedRate(1.seconds).repeat).use { cb =>
        cb.protect(IO.raiseError(new Exception())).attempt >>
          IO.sleep(2.seconds) >>
          cb.protect(IO(1)).void
      })
      ...
      .compile
      .lastOrError
      .map(ss => assert(ss.cause.exitCode == 0))
  }
