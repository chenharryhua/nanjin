...
class ServiceTest extends AnyFunSuite {
  ...
  test("16.blackbox should survive panic - option") {
    val List(a, b, c, d, e, f, g, h) = guard
      ...
      .eventStream { agent =>
        ...
        for {
          _ <- locker.get.flatMap {
            case None               => locker.set(Some(10))
            case Some(v) if v < 100 => locker.update(_.map(_ * v))
            case Some(_)            => locker.set(None)
          }
          _ <- locker.get
  ...
  test("17.blackbox should survive panic - initial value") {
  ...
  test("18.blackbox work as signal") {
    val List(a, b, c) =
      guard
        .withRestartPolicy(RetryPolicies.constantDelay[IO](0.1.seconds))
        .eventStream { agent =>
          ...
          agent
            ...
            .evalTap(_ => locker.update(_ + 1))
            .interruptWhen(locker.map(_ > 20))
            .compile
            .drain >> locker.get.flatMap(broker.passThrough(_))
        }
        .debug()
        .compile
        .toList
...
//  test("18.blackbox should survive panic - effectful initial value") {
//    val List(a, b, c, d, e, f, g, h) =
//      guard
//        .withRestartPolicy(RetryPolicies.constantDelay[IO](0.1.seconds))
//        .eventStream { agent =>
//          val locker = agent.blackbox(IO(10))
//          val broker = agent.broker("locker")
//          for {
//            _ <- locker.update(_ + 1)
//            v <- locker.get
