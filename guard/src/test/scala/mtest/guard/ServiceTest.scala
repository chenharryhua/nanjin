...
class ServiceTest extends CatsEffectSuite {
  ...
  test("1.should stopped if the operation normally exits") {
    guard.service("exit").eventStream(_ => IO(())).compile.toList.map { xs =>
  ...
  test("2.escalate to up level if retry failed") {
    guard
      ...
      .toList
      .map { xs =>
        val List(a, b, c, d, e, f, g, h) = xs: @unchecked
        assert(a.isInstanceOf[ServiceStart])
  ...
  test("9.by cancellation - internal") {
    guard.service("cancel").eventStream(_ => IO.unit <* IO.canceled).compile.toList.map { xs =>
      val List(a, b) = xs: @unchecked
      assert(a.isInstanceOf[ServiceStart])
