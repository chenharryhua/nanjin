...
class MetricsTest extends AnyFunSuite {
  ...
  test("delta") {
    val last = sg
      ...
      .eventStream(ag => ag.root("one").use(_.run(IO(0))) >> IO.sleep(10.minutes))
  ...
  test("full") {
    val last = sg
      ...
      .eventStream(ag =>
        ag.root("one").use(_.updateConfig(_.withCounting).run(IO(0))) >> IO.sleep(10.minutes))
  ...
  test("ongoing action alignment") {
    sg.updateConfig(_.withMetricSnapshotType(MetricSnapshotType.Regular).withMetricReport(1.second))
      .eventStream { ag =>
        val one = ag.root("one").use(_.expensive.run(IO(0) <* IO.sleep(10.minutes)))
        val two = ag.root("two").use(_.expensive.run(IO(0) <* IO.sleep(10.minutes)))
  ...
  test("reset") {
    val last = sg
      ...
      .eventStream { ag =>
        ...
        ag.root("one").use(_.run(IO(0))) >> ag
          .root("two")
          .use(_.run(IO(1))) >> metric.fullReport >> metric.reset >> IO.sleep(10.minutes)
      }
      .evalTap(console(Translator.simpleText[IO]))
      .map(_.asJson.noSpaces)
      .evalMap(e => IO(decode[NJEvent](e)).rethrow)
