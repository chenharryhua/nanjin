...
class MetricsTest extends AnyFunSuite {
  ...
  test("delta") {
    val last = sg
      ...
      .eventStream(ag => ag.action("one").run(IO(0)) >> IO.sleep(10.minutes))
  ...
  test("full") {
    val last = sg
      ...
      .eventStream(ag => ag.action("one").updateConfig(_.withCounting).run(IO(0)) >> IO.sleep(10.minutes))
  ...
  test("reset") {
    val last = sg
      ...
      .eventStream { ag =>
        ...
        ag.action("one").run(IO(0)) >> ag.action("two").run(IO(1)) >> metric.fullReport >> metric.reset >> IO
          .sleep(10.minutes)
      }
      .evalTap(console(Translator.simpleText[IO]))
      .map(_.asJson.noSpaces)
