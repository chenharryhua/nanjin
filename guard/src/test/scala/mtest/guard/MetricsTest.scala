...
class MetricsTest extends AnyFunSuite {
  ...
  test("delta") {
    val last = sg
      ...
      .eventStream(ag => ag.action(_.silent).retry(IO(0)).run("one") >> IO.sleep(10.minutes))
  ...
  test("full") {
    val last = sg
      ...
      .eventStream(ag => ag.action(_.withCounting).retry(IO(0)).run("one") >> IO.sleep(10.minutes))
  ...
  test("ongoing action alignment") {
    sg.updateConfig(_.withMetricSnapshotType(MetricSnapshotType.Regular).withMetricReport(1.second))
      .eventStream { ag =>
        val one = ag.action(_.notice).retry(IO(0) <* IO.sleep(10.minutes)).run("one")
        val two = ag.action(_.notice).retry(IO(0) <* IO.sleep(10.minutes)).run("two")
  ...
  test("reset") {
    val last = sg
      ...
      .eventStream { ag =>
        ...
        ag.action(_.notice).retry(IO(0)).run("one") >> ag
          .action(_.notice)
          .retry(IO(1))
          .run("two") >> metric.fullReport >> metric.reset >> IO.sleep(10.minutes)
      }
      .evalTap(console(Translator.simpleText[IO]))
      .map(_.asJson.noSpaces)
      .evalMap(e => IO(decode[NJEvent](e)).rethrow)
