...
import com.github.chenharryhua.nanjin.guard.event.{Domain, MetricLabel}
...
class BatchEncoderTest extends AnyFunSuite {
  ...
  test("quasi and value batches encode kind and result tags") {
    val quasi = QuasiBatch(label, Duration.ofMillis(20), BatchMode.Sequential, batchId,
      List(JobState(completed, Right(1))))
    val value = BatchValue(label, Duration.ofMillis(20), BatchMode.Sequential, batchId,
      List(JobValue(completed, 1)))
  ...
  test("failed results encode with the error tag") {
    val quasi: QuasiBatch[Int] = QuasiBatch(label, Duration.ofMillis(20), BatchMode.Sequential, batchId,
    ...
    assert(quasiJson.hcursor.downField("jobs").downArray.get[String]("error").toOption.exists(_.endsWith("boom")))
    assert(monadicJson.hcursor.downField("jobs").downArray.get[String]("job-1").toOption.contains("work"))
    assert(monadicJson.hcursor.downField("jobs").downArray.get[String]("failed").toOption.contains("critical"))
