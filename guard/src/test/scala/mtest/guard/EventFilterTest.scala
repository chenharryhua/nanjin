...
import com.github.chenharryhua.nanjin.guard.event.Event.{MetricsEvent, ServiceStart, ServiceStop}
...
class EventFilterTest extends AnyFunSuite {
  ...
  test("1.sampling - FiniteDuration") {
    val List(a, b, c, d) = service
      ...
      .toList
      .unsafeRunSync()
    val first = b.asInstanceOf[MetricsEvent].index.asInstanceOf[Periodic].tick.index
    assert(a.isInstanceOf[ServiceStart])
    assert(b.isInstanceOf[MetricsEvent])
    assert(c.asInstanceOf[MetricsEvent].index.asInstanceOf[Periodic].tick.index === first + 3)
  ...
  test("2.sampling - divisor") {
    val List(a, b, c, d) = service
      ...
      .unsafeRunSync()
    assert(a.isInstanceOf[ServiceStart])
    assert(b.asInstanceOf[MetricsEvent].index.asInstanceOf[Periodic].tick.index === 3)
    assert(c.asInstanceOf[MetricsEvent].index.asInstanceOf[Periodic].tick.index === 6)
    assert(d.isInstanceOf[ServiceStop])
  }
  ...
  test("3.sampling - cron") {
    ...
    val tb = b.asInstanceOf[MetricsEvent].index.asInstanceOf[Periodic].tick
    val tc = c.asInstanceOf[MetricsEvent].index.asInstanceOf[Periodic].tick
    assert(a.isInstanceOf[ServiceStart])
