...
class GaugeTest extends CatsEffectSuite {
  private val service =
  ...
  test("5.no policy gauge should recompute on each scrape") {
    ...
    }.map(checkJson).mapFilter(Event.metricsSnapshot.getOption).take(2).compile.toList.map { snapshots =>
  ...
  test("10.ratio gauge is collected as ratio data") {
    ...
    }.map(checkJson).mapFilter(Event.metricsSnapshot.getOption).take(1).compile.toList.map { snapshots =>
