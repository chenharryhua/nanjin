...
final private class NJMetricRegistry[F[_]](registry: MetricRegistry)(implicit F: Sync[F]) extends AlertService[F] {
  ...
  override def alert(event: NJEvent): F[Unit] = event match {
    case _: ServiceHealthCheck                        => F.delay(registry.counter(s"001.health.check").inc())
    case _: ServiceStarted                            => F.delay(registry.counter(s"002.service.start").inc())
    case _: ServiceStopped                            => F.delay(registry.counter(s"003.service.stop").inc())
    case _: ServicePanic                              => F.delay(registry.counter(s"004.service.panic").inc())
    ...
    case ActionFailed(at, info, _, _, _, _) =>
      F.delay(registry.timer(s"006.fail.[${info.actionName}]").update(Duration.between(info.launchTime, at)))
    case ActionRetrying(_, info, _, _, _) => F.delay(registry.counter(s"007.retry.[${info.actionName}]").inc())
    case _: ForYourInformation            => F.delay(registry.counter(s"008.fyi").inc())
