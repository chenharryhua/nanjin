...
final private class SlackSink[F[_]](snsResource: Resource[F, SimpleNotificationService[F]])(implicit F: Sync[F])
  ...
  private def send(event: NJEvent, sns: SimpleNotificationService[F]): F[Unit] =
    event match {
      ...
      case ActionStart(params, action, at, _) =>
        def msg: String =
          SlackNotification(
      ...
      case ActionSucced(params, action, at, _, numRetries, notes) =>
        def msg: String =
          SlackNotification(
            params.serviceParams.taskParams.appName,
            notes.value,
      ...
      case ActionQuasiSucced(params, action, at, _, runMode, numSucc, succNotes, failNotes, errors) =>
