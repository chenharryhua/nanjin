...
final class JsonLogging[F[_]: Sync] private[observers] (translator: Translator[F, Json]) extends (NJEvent => F[Unit]) {
  def updateTranslator(f: Translator[F, Json] => Translator[F, Json]) = new JsonLogging[F](f(translator))
    ...
    event match {
      case sp @ ServicePanic(_, _, _, error) =>
        translator.servicePanic
          .run(sp)
          .value
          .flatMap(oj => (oj, error.throwable).traverseN { case (j, t) => logger.error(t)(j.spaces2) }.void)
      ...
      case ar @ ActionRetrying(_, _, _, error) =>
        translator.actionRetrying
          .run(ar)
          .value
          .flatMap(oj => (oj, error.throwable).traverseN { case (j, t) => logger.warn(t)(j.spaces2) }.void)
...
final class TextLogging[F[_]: Sync] private[observers] (translator: Translator[F, String])
  ...
  def updateTranslator(f: Translator[F, String] => Translator[F, String]) = new TextLogging[F](f(translator))
    ...
    event match {
      case sp @ ServicePanic(_, _, _, error) =>
        translator.servicePanic
          .run(sp)
          .value
          .flatMap(oj => (oj, error.throwable).traverseN { case (j, t) => logger.error(t)(j) }.void)
      case sa: ServiceAlert =>
      ...
      case ar @ ActionRetrying(_, _, _, error) =>
        translator.actionRetrying
          .run(ar)
          .value
          .flatMap(oj => (oj, error.throwable).traverseN { case (j, t) => logger.warn(t)(j) }.void)
      case af @ ActionFailed(_, _, _, _, error) =>
        translator.actionFailed
          .run(af)
          .value
