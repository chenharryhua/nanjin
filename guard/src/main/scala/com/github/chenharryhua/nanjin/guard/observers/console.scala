...
object console {
  def json[F[_]: Console: Monad](f: Json => String): JsonConsole[F] =
    new JsonConsole[F](Translator.json[F], f)
...
final class JsonConsole[F[_]: Monad] private[observers] (
  ...
  def updateTranslator(f: Translator[F, Json] => Translator[F, Json]) = new JsonConsole[F](f(translator), jsonConverter)
