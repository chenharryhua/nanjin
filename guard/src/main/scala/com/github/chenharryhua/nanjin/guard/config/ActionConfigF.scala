...
@Lenses final case class ActionRetryParams private (
  ...
  def policy[F[_]: Applicative]: RetryPolicy[F] =
    capDelay.fold(njRetryPolicy.policy[F].join(RetryPolicies.limitRetries(maxRetries)))(cd =>
      RetryPolicies.capDelay(cd, njRetryPolicy.policy[F]).join(RetryPolicies.limitRetries(maxRetries)))
}
