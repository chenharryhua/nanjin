...
import com.github.chenharryhua.nanjin.guard.alert.{AlertService, NJEmptyReporter}
...
final class TaskGuard[F[_]: Async] private (taskConfig: TaskConfig, alertServices: Resource[F, AlertService[F]])
  ...
  def service(serviceName: String): ServiceGuard[F] =
    new ServiceGuard[F](ServiceConfig(serviceName, params), alertServices, new NJEmptyReporter)
