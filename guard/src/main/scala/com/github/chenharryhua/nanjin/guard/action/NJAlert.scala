...
final class NJAlert[F[_]: Temporal] private[guard] (
  ...
  def error[S: Show](msg: S): F[Unit] =
    publisher.alert(metricName, msg.show, Importance.Critical).map(_ => errorCounter.inc(1))
  ...
  def warn[S: Show](msg: S): F[Unit] =
    publisher.alert(metricName, msg.show, Importance.High).map(_ => warnCounter.inc(1))
  ...
  def info[S: Show](msg: S): F[Unit] =
    publisher.alert(metricName, msg.show, Importance.Medium).map(_ => infoCounter.inc(1))
