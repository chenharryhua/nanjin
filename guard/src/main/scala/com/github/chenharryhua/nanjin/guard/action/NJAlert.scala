...
final class NJAlert[F[_]: Monad: Clock] private[guard] (
  ...
  def error[S: Encoder](msg: S, ifTrue: Boolean): F[Unit] = if (ifTrue) error(msg) else Monad[F].unit
  ...
  def unsafeError[S: Encoder](msg: S, ifTrue: Boolean): Unit = if (ifTrue) unsafeError(msg) else ()
  ...
  def warn[S: Encoder](msg: S, ifTrue: Boolean): F[Unit] = if (ifTrue) warn(msg) else Monad[F].unit
  ...
  def unsafeWarn[S: Encoder](msg: S, ifTrue: Boolean): Unit = if (ifTrue) unsafeWarn(msg) else ()
