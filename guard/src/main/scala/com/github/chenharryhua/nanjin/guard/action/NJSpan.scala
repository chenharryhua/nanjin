...
final class NJSpan[F[_]] private[guard] (rawSpan: Span[F])(implicit F: Temporal[F]) extends Span[F] {
  override def put(fields: (String, TraceValue)*): F[Unit] = rawSpan.put(fields*)
  override def kernel: F[Kernel]                           = rawSpan.kernel
  override def span(name: String): Resource[F, NJSpan[F]]  = rawSpan.span(name).map(new NJSpan[F](_))
