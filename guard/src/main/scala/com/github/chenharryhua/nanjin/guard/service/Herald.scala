...
abstract private class HeraldImpl[F[_]: Sync](
  ...
  override def info[S: Encoder](msg: S): F[Unit] =
    for {
      evt <- service_message[F, S](serviceParams, msg, AlarmLevel.Info, None)
  ...
  override def done[S: Encoder](msg: S): F[Unit] =
    for {
      evt <- service_message[F, S](serviceParams, msg, AlarmLevel.Done, None)
      _ <- eventLogger.logServiceMessage(evt)
      _ <- channel.send(evt)
    } yield ()
  ...
  override def warn[S: Encoder](msg: S): F[Unit] =
    for {
      evt <- service_message[F, S](serviceParams, msg, AlarmLevel.Warn, None)
      _ <- eventLogger.logServiceMessage(evt)
      _ <- channel.send(evt)
    } yield ()
  ...
  override def warn[S: Encoder](ex: Throwable)(msg: S): F[Unit] =
    for {
      evt <- service_message[F, S](serviceParams, msg, AlarmLevel.Warn, Some(Error(ex)))
      _ <- eventLogger.logServiceMessage(evt)
      _ <- channel.send(evt)
    } yield ()
  ...
  override def error[S: Encoder](msg: S): F[Unit] =
    for {
      evt <- service_message[F, S](serviceParams, msg, AlarmLevel.Error, None)
  ...
  override def error[S: Encoder](ex: Throwable)(msg: S): F[Unit] =
    for {
      evt <- service_message[F, S](serviceParams, msg, AlarmLevel.Error, Error(ex).some)
      _ <- errorHistory.modify(queue => (queue, queue.add(evt)))
      _ <- eventLogger.logServiceMessage(evt)
      _ <- channel.send(evt)
    } yield ()
