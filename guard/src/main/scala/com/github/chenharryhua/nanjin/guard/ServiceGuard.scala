...
final class ServiceGuard[F[_]] private[guard] (
  ...
  reporters: List[NJMetricsReporter])(implicit F: Async[F])
    ...
    for {
      ...
      event <- Stream.eval(Channel.unbounded[F, NJEvent]).flatMap { channel =>
        ...
        val notify: Pipe[F, NJEvent, INothing] = { events =>
          val alerts: Resource[F, AlertService[F]] =
            reporters
              .map(_.resource[F](metricRegistry))
              .foldLeft(alertServices) { case (sum, rp) =>
                sum.flatMap(s => rp.map(_ |+| s))
              }
              .map(_ |+| new NJMetricRegistry[F](metricRegistry))
