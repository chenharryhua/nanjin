...
final class ServiceGuard[F[_]] private[guard] (
  ...
  def eventStream[A](agent: Agent[F] => F[A]): Stream[F, NJEvent] =
    for {
      ...
      runningCriticalActions <- Stream.eval(F.ref(Set.empty[ActionInfo]))
      event <- Stream.eval(Channel.bounded[F, NJEvent](params.queueCapacity)).flatMap { channel =>
        val metricRegistry: MetricRegistry = new MetricRegistry()
        val publisher: EventPublisher[F] =
          new EventPublisher[F](serviceInfo, metricRegistry, lastCountersRef, runningCriticalActions, channel)
