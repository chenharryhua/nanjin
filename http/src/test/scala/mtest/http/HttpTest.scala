...
class HttpTest extends AnyFunSuite {
  ...
  test("cookie box") {
    ...
    server
      .surround(client.use(_.expect[String]("http://0.0.0.0:8080/cookie").flatMap(IO.println)))
      .unsafeRunSync()
  }
  ...
  test("trace") {
    ...
    server
      .surround(client.use(_.expect[String]("http://0.0.0.0:8080/trace/world").flatMap(IO.println)))
      .unsafeRunSync()
  }
