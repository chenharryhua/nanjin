...
@Lenses final case class AuthParams(
  ...
  logHeader: Boolean,
  logBody: Boolean) {
  def offset: FiniteDuration = maxWait * maxRetries.toLong
...
object AuthParams {
  def apply(expiresIn: FiniteDuration): AuthParams =
    AuthParams(maxRetries = 8, maxWait = 5.second, expiresIn = expiresIn, logHeader = false, logBody = false)
}
...
final private[auth] case class AuthConfig private (value: Fix[AuthConfigF]) {
  ...
  def withAuthHeaderLog: AuthConfig = AuthConfig(Fix(WithAuthHeaderLog(value = true, value)))
  def withAuthBodyLog: AuthConfig   = AuthConfig(Fix(WithAuthBodyLog(value = true, value)))
