...
class ConsumeMessageAkkaTest extends FunSuite with ShowKafkaMessage with AkkaMessageBitraverse {
  ...
  test("akka stream should be able to consume data") {
    val chn = ctx.topic(vessel).akkaChannel.updateConsumerSettings(s => s.withClientId("id"))
    chn.consumeValidMessages.map(_.show).map(println).take(3).runWith(Sink.ignore)(chn.materializer)
  }
}
