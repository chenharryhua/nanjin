...
class SerdeTest extends CatsEffectSuite {
  ...
  test("3.null - deserialize") {
    ...
    intercept[NullPointerException](ctx.asValue(s1).serde.deserializer().deserialize("t", null))
  ...
  test("4.null - serialize") {
    val s1 = Structured[GenericRecord].become[Foo]
    intercept[NullPointerException](ctx.asValue(s1).serde.serializer().serialize("t", null))
    val s2 = Structured[GenericRecord].option.become[Option[Foo]].orNull
  ...
  test("5.attempt - should not throw exception") {
    ...
    ctx.asValue(s1).deserializer[IO].map(_.attempt).use(
      _.deserialize("a", Headers.empty, Array(1, 2, 3))).map { deSer =>
      assert(deSer.isLeft)
    }
  }
  ...
  test("6.primitive") {
    ...
    for {
      serialized <- ctx.asValue(s1).serializer[IO].use(_.serialize("a", Headers.empty, Some(10)))
      y <- ctx.asValue(s1).serializer[IO].use(_.serialize("a", Headers.empty, None))
    } yield {
      val List(_, _, _, x) = serialized.toList: @unchecked
      assert(x == 10)
      assert(y == null)
    }
  }
