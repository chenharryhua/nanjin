  ...
  test("should compile") {
    ...
    val task =
      topic.admin.IdefinitelyWantToDeleteTheTopicAndUnderstoodItsConsequence >>
        ...
        ret.runWith(chn.ignoreSink) *> IO(println("-----------1-------------"))
      } >> {
        ...
        ret.runWith(chn.ignoreSink) *> IO(println("-----------2-------------"))
      } >> {
        ...
        ret.runWith(chn.ignoreSink) *> IO(println("-----------3-------------"))
      } >> {
        val ret
        ...
        ret.runWith(chn.ignoreSink) *> IO(println("-----------4-------------"))
      } >> {
        ...
        ret.runWith(chn.ignoreSink) *> IO(println("-----------5-------------"))
      } >> {
        val ret
        ...
        ret.runWith(chn.ignoreSink) *> IO(println("-----------6-------------"))
      } >> {
        ...
        ret.runWith(chn.ignoreSink) *> IO(println("-----------7-------------"))
      }
    task.unsafeRunSync()
  }
}
