...
import com.github.chenharryhua.nanjin.kafka.{KafkaSerdeBuilder, TopicDef}
...
object apps {
  def kafka_streaming(sb: StreamsBuilder, rs: KafkaSerdeBuilder): Unit = {
  ...
  def kafka_streaming_bad_record(sb: StreamsBuilder, rs: KafkaSerdeBuilder): Unit = {
  ...
  def transformer_app(sbb: StreamsBuilder, ksb: KafkaSerdeBuilder): Unit = {
