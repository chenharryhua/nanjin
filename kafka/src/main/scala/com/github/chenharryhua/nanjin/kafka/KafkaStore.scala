...
sealed abstract class KafkaStore[K, V, Q[_, _], S <: StateStore](
  val storeName: KafkaStoreName,
  val key: SerdeAndSchema[K],
  val value: SerdeAndSchema[V]
) {
...
object KafkaStore {
  ...
  final class Persistent[K, V](storeName: KafkaStoreName, key: SerdeAndSchema[K], value: SerdeAndSchema[V])
    ...
    override val materialized: Materialized[K, V, scala.ByteArrayKeyValueStore] =
      Materialized.as(Stores.persistentKeyValueStore(storeName.value))(key.serde, value.serde)
  }

  final class InMemory[K, V](storeName: KafkaStoreName, key: SerdeAndSchema[K], value: SerdeAndSchema[V])
      extends KafkaStore[K, V, ReadOnlyKeyValueStore, scala.ByteArrayKeyValueStore](
        storeName,
        key,
        value) {
    ...
    override val materialized: Materialized[K, V, scala.ByteArrayKeyValueStore] =
      Materialized.as(Stores.inMemoryKeyValueStore(storeName.value))(key.serde, value.serde)
  }

  final class Session[K, V](
    storeName: KafkaStoreName,
    key: SerdeAndSchema[K],
    value: SerdeAndSchema[V],
