...
sealed abstract class KafkaStore[K, V, Q[_, _], S <: StateStore](storeName: StoreName) extends Serializable {
...
object KafkaStore {
  ...
  final class Persistent[K, V](storeName: StoreName, keySerde: SerdeOf[K], valueSerde: SerdeOf[V])
  ...
  final class InMemory[K, V](storeName: StoreName, keySerde: SerdeOf[K], valueSerde: SerdeOf[V])
      extends KafkaStore[K, V, ReadOnlyKeyValueStore, scala.ByteArrayKeyValueStore](storeName) {
  ...
  final class Session[K, V](
    ...
    override val materialized: Materialized[K, V, scala.ByteArraySessionStore] =
      Materialized.as(Stores.persistentSessionStore(storeName.value, retentionPeriod))(keySerde, valueSerde)
  }
