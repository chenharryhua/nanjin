...
final class StreamingChannel[F[_], K, V] private[kafka] (topic: KafkaTopic[F, K, V]) {
  ...
  val consumed: Consumed[K, V] = Consumed.`with`[K, V](topic.registered.keySerde, topic.registered.valSerde)
  val produced: Produced[K, V] = Produced.`with`[K, V](topic.registered.keySerde, topic.registered.valSerde)
  ...
  def ktable(supplier: KeyValueBytesStoreSupplier): Reader[StreamsBuilder, KTable[K, V]] =
    ktable(Materialized.as[K, V](supplier)(topic.registered.keySerde, topic.registered.valSerde))
  ...
  def gktable(supplier: KeyValueBytesStoreSupplier): Reader[StreamsBuilder, GlobalKTable[K, V]] =
    gktable(Materialized.as[K, V](supplier)(topic.registered.keySerde, topic.registered.valSerde))
}
