...
"""
  ...
  test("should be able to parse sqs S3 event") {
    val s3 = sqsS3Parser(SqsMessage(ReceiveMessageRequest.builder().build(), Message.builder().body(event).build(), 0, 0, 0)).head
  ...
  test("nulls") {
    ...
    assert(sqsS3Parser(SqsMessage(ReceiveMessageRequest.builder().build(),  Message.builder().build(), 0, 0, 0)).isEmpty)
