...
sealed trait SimpleEmailService[F[_]] {
  def send(txt: EmailContent): F[SendEmailResponse]
...
object SimpleEmailService {
  ...
  def fake[F[_]](implicit F: Sync[F]): Resource[F, SimpleEmailService[F]] = {
    ...
    Resource.make(F.pure(new SimpleEmailService[F] {
      override def send(txt: EmailContent): F[SendEmailResponse] =
        logger.info(txt.show) *> F.pure(SendEmailResponse.builder().messageId("fake.message.id").build())
  ...
  final private class AwsSES[F[_]](buildFrom: Endo[SesClientBuilder], logger: Logger[F])(implicit F: Sync[F])
    ...
    override def send(content: EmailContent): F[SendEmailResponse] = {
      val request: SendEmailRequest.Builder = SendEmailRequest
        .builder()
        ...
        .message(
          Message
            ...
            .build())
      F.blocking(client.sendEmail(request.build())).onError(ex => logger.error(ex)(request.toString))
    }
