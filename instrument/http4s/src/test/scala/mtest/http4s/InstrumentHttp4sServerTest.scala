...
class InstrumentHttp4sServerTest extends AnyFunSuite {
  ...
  private val server: Resource[IO, Server] = EmberServerBuilder
    ...
    .withPort(port"8080")
    .withHttpApp(Router("/" -> Metrics[IO](listener)(service)).orNotFound)
    .build

  val client: Resource[IO, Client[IO]] = EmberClientBuilder.default[IO].build

  test("http4s server instrument") {
    TaskGuard[IO]("http4s")
      .service("server metrics")
      .eventStream { ga =>
        (server >> instrumentHttp4s(ga, listener, "http4s") >> client).use { c =>
          val get =
