...
/** @param sessionId
  ...
  */
@JsonCodec
final case class Tick private[datetime] (
  sessionId: UUID,
  snooze: Duration,
  previous: Instant,
  wakeTime: Instant,
...
object awakeOnPolicy {
  ...
  final def apply[F[_]](policy: RetryPolicy[F], initTick: Tick)(implicit F: Temporal[F]): Stream[F, Tick] =
  ...
  final def apply[F[_]: UUIDGen](policy: RetryPolicy[F])(implicit F: Temporal[F]): Stream[F, Tick] =
