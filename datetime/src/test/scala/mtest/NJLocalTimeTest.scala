...
class NJLocalTimeTest extends AnyFunSuite {
  ...
  test("local time range - do not cross midnight") {
    ...
    val d3 = LocalDateTime.of(2013, 9, 23, 15, 0, 0).atZone(sydneyTime)
  ...
  test("local time range - cross midnight") {
    ...
    val d4 = LocalDateTime.of(2015, 4, 4, 0, 0, 0).atZone(sydneyTime)
    val d5 = LocalDateTime.of(2015, 4, 4, 0, 0, 1).atZone(sydneyTime)
    val d6 = LocalDateTime.of(2015, 4, 4, 12, 0, 0).atZone(sydneyTime)
    assert(ltr.inBetween(d3))
    assert(ltr.inBetween(d4))
    assert(ltr.inBetween(d5))
  ...
  test("local time range - start time match") {
    ...
    val d3 = LocalDateTime.of(2014, 3, 3, 23, 59, 59).atZone(sydneyTime)
    val d4 = LocalDateTime.of(2014, 3, 3, 1, 0, 0).atZone(sydneyTime)
    assert(ltr.inBetween(d3))
    assert(!ltr.inBetween(d4))
  }
