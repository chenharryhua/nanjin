...
final class LoadTable[F[_], A] private[spark] (ate: AvroTypedEncoder[A], ss: SparkSession)(implicit
  ...
  def empty: NJTable[F, A] = new NJTable[F, A](F.pure(ate.emptyDataset(ss)), ate)

  def parquet(path: NJPath): NJTable[F, A] =
    new NJTable[F, A](F.blocking(loaders.parquet[A](path, ss, ate)), ate)

  def avro(path: NJPath): NJTable[F, A] =
    new NJTable[F, A](F.blocking(loaders.avro[A](path, ss, ate)), ate)

  def circe(path: NJPath)(implicit ev: JsonDecoder[A]): NJTable[F, A] =
    new NJTable[F, A](F.blocking(loaders.circe[A](path, ss, ate)), ate)

  def kantan(path: NJPath, cfg: CsvConfiguration)(implicit dec: RowDecoder[A]): NJTable[F, A] =
    new NJTable[F, A](F.blocking(loaders.kantan[A](path, ss, ate, cfg)), ate)

  def jackson(path: NJPath): NJTable[F, A] =
    new NJTable[F, A](F.blocking(loaders.jackson[A](path, ss, ate)), ate)

  def binAvro(path: NJPath): NJTable[F, A] =
    new NJTable[F, A](F.blocking(loaders.binAvro[A](path, ss, ate)), ate)

  def objectFile(path: NJPath): NJTable[F, A] =
    new NJTable[F, A](F.blocking(loaders.objectFile(path, ss, ate)), ate)

  object spark {
    def json(path: NJPath): NJTable[F, A] =
      new NJTable[F, A](F.blocking(loaders.spark.json[A](path, ss, ate)), ate)

    def parquet(path: NJPath): NJTable[F, A] =
      new NJTable[F, A](F.blocking(loaders.spark.parquet[A](path, ss, ate)), ate)

    def avro(path: NJPath): NJTable[F, A] =
      new NJTable[F, A](F.blocking(loaders.spark.avro[A](path, ss, ate)), ate)

    def csv(path: NJPath, cfg: CsvConfiguration): NJTable[F, A] =
      new NJTable[F, A](F.blocking(loaders.spark.csv[A](path, ss, ate, cfg)), ate)
  ...
  def jdbc(hikari: HikariConfig, query: TableQuery): NJTable[F, A] = {
    ...
    new NJTable[F, A](F.blocking(ate.normalizeDF(ss.read.format("jdbc").options(sparkOptions).load())), ate)
  }
