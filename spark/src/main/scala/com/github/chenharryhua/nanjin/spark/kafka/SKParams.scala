...
@Lenses final case class KitBundle[K, V](kit: KafkaTopicKit[K, V], params: SKParams) {
  ...
  def withTimeRange(f: NJDateTimeRange => NJDateTimeRange): KitBundle[K, V] =
    KitBundle.params.composeLens(SKParams.timeRange).modify(f)(this)

  def zoneId: ZoneId = params.timeRange.zoneId
  def clock: Clock   = Clock.system(zoneId)

  def withSaveMode(sm: SaveMode): KitBundle[K, V] =
    KitBundle.params.composeLens(SKParams.saveMode).set(sm)(this)

  def withOverwrite: KitBundle[K, V] =
    KitBundle.params.composeLens(SKParams.saveMode).set(SaveMode.Overwrite)(this)

  def withPathBuilder(pb: KafkaPathBuild => String): KitBundle[K, V] =
    KitBundle.params.composeLens(SKParams.pathBuilder).set(Reader(pb))(this)

  def withFileFormat(ff: NJFileFormat): KitBundle[K, V] =
    KitBundle.params.composeLens(SKParams.fileFormat).set(ff)(this)
  ...
  def withLocationStrategy(ls: LocationStrategy): KitBundle[K, V] =
    KitBundle.params.composeLens(SKParams.locationStrategy).set(ls)(this)

  def withBatchSize(batchSize: Int): KitBundle[K, V] =
    KitBundle.params
      .composeLens(SKParams.uploadRate)
  ...
  def withRepartition(num: Int): KitBundle[K, V] =
    KitBundle.params.composeLens(SKParams.repartition).set(NJRepartition(num))(this)

  def withRowNumber(num: Int): KitBundle[K, V] =
    KitBundle.params.composeLens(SKParams.showDs).composeLens(NJShowDataset.rowNum).set(num)(this)

  def withTruncate: KitBundle[K, V] =
    KitBundle.params
      .composeLens(SKParams.showDs)
