...
final class TableDS[F[_], A] private[database] (
  ...
  def map[B](f: A => B)(tdb: TableDef[B]): TableDS[F, B] =
    new TableDS[F, B](dataset.map(f)(tdb.ate.sparkEncoder), tdb, dbSettings, cfg).normalize

  def flatMap[B](f: A => IterableOnce[B])(tdb: TableDef[B]): TableDS[F, B] =
    new TableDS[F, B](dataset.flatMap(f)(tdb.ate.sparkEncoder), tdb, dbSettings, cfg).normalize
