...
// 1
  def jackson[A](pathStr: String)(implicit
    ...
    new NJHadoop[F](conf).byteStream(pathStr, blocker).through(pipe.deserialize).through(gr.decode)
  }
...
// 2
  def circe[A: JsonDecoder](
    ...
    new NJHadoop[F](conf).byteStream(pathStr, blocker).through(pipe.deserialize)
  }
...
// 11
  def parquet[A: TypedEncoder](pathStr: String)(implicit
    ...
    new NJHadoop[F](conf).parquetSource(pathStr, dec.schema, blocker).through(pipe.decode)
  }
...
// 12
  def avro[A](pathStr: String)(implicit
    ...
    ce: ConcurrentEffect[F]): Stream[F, A] = {
    val pipe = new GenericRecordCodec[F, A]
    new NJHadoop[F](conf).avroSource(pathStr, dec.schema, blocker).through(pipe.decode)
  }
