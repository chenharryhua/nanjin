...
object sinks {
  ...
  def avro[F[_]: Sync, A](
    ...
    cf: CodecFactory): Pipe[F, A, INothing] = { (ss: Stream[F, A]) =>
    val grc: Pipe[F, A, GenericRecord]         = new GenericRecordCodec[F, A].encode(encoder)
    val sink: Pipe[F, GenericRecord, INothing] = NJHadoop[F](cfg).avroSink(path, encoder.schema, cf)
  ...
  def parquet[F[_]: Sync, A](
    ...
    cfg: Configuration,
    encoder: AvroEncoder[A],
    ccn: CompressionCodecName): Pipe[F, A, INothing] = { (ss: Stream[F, A]) =>
    val grc: Pipe[F, A, GenericRecord]         = new GenericRecordCodec[F, A].encode(encoder)
    val sink: Pipe[F, GenericRecord, INothing] = NJHadoop[F](cfg).parquetSink(path, encoder.schema, ccn)
    ss.through(grc).through(sink)
  }
