...
object TableDef {
  ...
  def apply[A](tableName: TableName, codec: NJAvroCodec[A])(implicit typedEncoder: TypedEncoder[A]) =
    new TableDef[A](tableName, AvroTypedEncoder(codec), None)

  def apply[A](tableName: TableName, codec: NJAvroCodec[A], unloadQuery: String)(implicit
