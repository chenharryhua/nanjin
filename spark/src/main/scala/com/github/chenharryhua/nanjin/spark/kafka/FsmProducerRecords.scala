...
final class FsmProducerRecords[F[_], K: TypedEncoder, V: TypedEncoder](
  ...
  def withBatchSize(num: Int) =
    new FsmProducerRecords[F, K, V](prs, kit, ConfigParamF.withBatchSize(num, params))

  def withDuration(fd: FiniteDuration) =
    new FsmProducerRecords[F, K, V](prs, kit, ConfigParamF.withDuration(fd, params))

  def withRepartition(rp: Int) =
    new FsmProducerRecords[F, K, V](prs, kit, ConfigParamF.withRepartition(rp, params))
