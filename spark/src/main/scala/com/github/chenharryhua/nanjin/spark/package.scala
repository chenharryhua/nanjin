...
package object spark {
  ...
  implicit final class SparkSessionExt(ss: SparkSession) extends Serializable {
    ...
    def loadDataset[A: Encoder](ate: Url): LoadDataset[A] = new LoadDataset[A](ate, ss)
    def loadData[G[_]: Foldable, A: Encoder](ga: G[A]): Dataset[A] = ss.createDataset(ga.toList)

    def loadProtobuf[A <: GeneratedMessage: ClassTag: GeneratedMessageCompanion](path: Url): RDD[A] =
      loaders.protobuf[A](path, ss)
