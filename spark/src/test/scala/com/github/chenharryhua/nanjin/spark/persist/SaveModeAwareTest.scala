...
class SaveModeAwareTest extends AnyFunSuite {
  ...
  test("error if exists") {
    val sma = new SaveModeAware[IO](SaveMode.ErrorIfExists, NJPath("./data"), hadoopConfig)
    assertThrows[Exception](sma.checkAndRun(IO(())).unsafeRunSync())
  }

  test("ignore if exists") {
    val sma = new SaveModeAware[IO](SaveMode.Ignore, NJPath("./data"), hadoopConfig)
    sma.checkAndRun(IO(())).unsafeRunSync()
  }

  test("overwrite if exists") {
    val path = NJPath("./data/test/spark/sma/overwrite.json")
    val file = File(path.pathStr)
