...
class KantanCsvTest extends AnyFunSuite {
  ...
  def saver(path: NJPath): SaveKantanCsv[IO, Tablet] = new RddFileHoarder[IO, Tablet](rdd).kantan(path)
  ...
  test("tablet read/write identity multi.uncompressed") {
    ...
    s.run.unsafeRunSync()
    val t = loaders.rdd.kantan[Tablet](path, sparkSession, s.csvConfiguration)
  ...
  test("tablet read/write identity multi.gzip") {
    ...
    s.run.unsafeRunSync()
    val t = loaders.rdd.kantan[Tablet](path, sparkSession, CsvConfiguration.rfc)
    assert(data.toSet == t.collect().toSet)
    assert(data.toSet == loadTablet(path, s.csvConfiguration).unsafeRunSync())
  }
  ...
  test("tablet read/write identity multi.1.deflate") {
    ...
    s.run.unsafeRunSync()
    val t  = loaders.rdd.kantan[Tablet](path, sparkSession, CsvConfiguration.rfc).collect().toSet
...
//  }
  ...
  test("tablet read/write identity with-header-delimiter") {
    ...
    s.run.unsafeRunSync()
    val t = loaders.rdd.kantan[Tablet](path, sparkSession, rfc)
    assert(data.toSet == t.collect().toSet)
    assert(data.toSet == loadTablet(path, s.csvConfiguration).unsafeRunSync())
  }
