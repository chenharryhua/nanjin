...
@DoNotDiscover
class SparkKafkaStreamTest extends AsyncFreeSpec with AsyncIOSpec with Matchers {
  ...
  "Kafka - Spark Structured Stream" - {
    ...
    "console sink" in {
      ...
      ss.concurrently(upload).interruptAfter(10.seconds).compile.drain.assertNoException
    }

    "file sink avro - should be read back" in {
      ...
      (ss.concurrently(upload).interruptAfter(6.seconds).compile.drain >>
        sparKafka.topic(rooster).load.avro(path).count.map(println)).assertNoException
    }

    "date partition sink json - should be read back" ignore {
      ...
      sparKafka.topic(rooster).load.json(todayPath).count.map(println).assertNoException
    }

    "memory sink - validate kafka timestamp" ignore {
