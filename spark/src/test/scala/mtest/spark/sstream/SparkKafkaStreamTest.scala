...
class SparkKafkaStreamTest extends AnyFunSuite {
  ...
  test("console sink") {
    ...
    val upload =
      sparKafka.topic(rooster).prRdd(data).withInterval(0.5.seconds).upload.withChunkSize(1).stream.delayBy(2.second)
  ...
  test("memory sink - validate kafka timestamp") {
    ...
    val upload =
      sparKafka.topic(rooster).prRdd(data).withInterval(1.second).upload.withChunkSize(6).stream.delayBy(3.second)
    ss.concurrently(upload).interruptAfter(6.seconds).compile.drain.unsafeRunSync()
