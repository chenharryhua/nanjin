...
class SaveDateTimeTest extends AnyFunSuite {
  ...
  test("sparKafka datetime single jackson") {
    ...
    val action = sk(path).fromKafka.flatMap(_.save.jackson(path).single.run[IO](blocker))
  ...
  test("sparKafka datetime single circe json") {
    ...
    val action =
      sk(path).fromKafka.flatMap(_.rdd.save.circe(path).single.run[IO](blocker))
    action.unsafeRunSync()
  ...
  test("sparKafka datetime single avro") {
    ...
    val action = sk(path).fromKafka.flatMap(_.save.avro(path).single.run[IO](blocker))
    action.unsafeRunSync()
  ...
  test("sparKafka datetime multi avro") {
    ...
    val action = sk(path).fromKafka.flatMap(_.save.avro(path).multi.run[IO](blocker))
    action.unsafeRunSync()
    val rst: Array[OptionalKV[Int, Chicken]] =
      sparkSession.load.avro[OptionalKV[Int, Chicken]](path).collect().sorted
    assert(rst.flatMap(_.value).toList == chickens)
  ...
  test("sparKafka datetime multi jackson") {
    ...
    val action = sk(path).fromKafka.flatMap(_.save.jackson(path).multi.run[IO](blocker))
    action.unsafeRunSync()
    val rst: Array[OptionalKV[Int, Chicken]] =
