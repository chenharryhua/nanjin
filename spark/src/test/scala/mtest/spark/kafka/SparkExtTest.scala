...
class SparkExtTest extends AnyFunSuite {
  ...
  test("stream") {
    topic.sparKafka.fromKafka.flatMap(_.sorted.stream[IO].compile.drain).unsafeRunSync
  }
  test("source") {
    topic.sparKafka.fromKafka
      .flatMap(_.sorted.source[IO].map(println).take(10).runWith(akkaSinks.ignore[IO]))
      .unsafeRunSync
  }
}
