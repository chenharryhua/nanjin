...
class KafkaAvroTest extends AnyFunSuite {
  ...
  test("sparKafka should be sent to kafka and save to single avro") {
    ...
    val avro = sk.load.stream.avro(avroPath).mapFilter(_.value).compile.toList.unsafeRunSync().toSet
    assert(avro == Set(en1, en2))

    val jackson = sk.load.stream.jackson(jacksonPath).mapFilter(_.value).compile.toList.unsafeRunSync().toSet
    assert(jackson == Set(en1, en2))

    val circe = sk.load.stream.circe(circePath).mapFilter(_.value).compile.toList.unsafeRunSync().toSet
